...
: mCurrentSampleRate (44100.0f), mQ (0.1f), mCutoff (1000.0f)
, mGlobalBypass (false)
...
void viator::SVFilter<SampleType>::setParameter(ParameterId parameter, SampleType parameterValue)
{
    switch (parameter)
    {
        ...
        {
            mCurrentSampleRate = juce::jlimit(1.0f, 3072000.0f, static_cast<float>(parameterValue));
            setSampleRates();
            break;
        }
...
{
    mGain.setTargetValue(pow(10, value * 0.05) - 1.f);
    mRawGain = value;
}
...
{
    return juce::Decibels::decibelsToGain(std::abs(value)) * 0.25f - 0.24f;
}

template <typename SampleType>
SampleType viator::SVFilter<SampleType>::getPeakQ(SampleType value) const
{
    return juce::Decibels::decibelsToGain(std::abs(value)) * 0.1f;
}
